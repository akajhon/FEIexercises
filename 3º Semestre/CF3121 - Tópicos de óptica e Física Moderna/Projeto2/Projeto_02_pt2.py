...
""")
while True:
    entrada = (input("\n====================================\n***** Escolha uma das opções: *****\n[S] - Sair\n[C] - Entrar com Comprimento de onda\n[F] - Entrar com Frequência\n[Q] - Entrar com Número Quântico\n[foton] - entrar com n final e n inicial\n[B] - entra com n inicial, escolha uma das 5 serie do hidrogenio\n====================================\n->"))
    ...
    #Nível Quantico
    ...
    elif(entrada == 'foton' or entrada == 'Foton'):
        n1 = 0
        n2 = 0
        ...
        while(n1 < 1 or n2 < 1):
            ...
            if(n1 < 1):
                ...
                print("\n[*] ATENÇÃO [*] o 'n' final e o n inicial não podem ser iguais!!!")
                n1 = int(input("\n[+] Digite o 'n' Inicial: "))
                n2 = int(input("\n[+] Digite o 'n' Final: "))
            
        if(n1 < n2):
            compFoton, freqFoton = calculaFoton(n1,n2)
            ...
            compFoton, freqFoton = calculaFoton(n1,n2)#Nas linhas 160 e 161 salvam em uma variavel auxiliar o retorno da função
            tipo_onda = calc_tipo_onda(freqFoton)
        ...
        while(n1 < 2):
            n1 = int(input("\n[+] Digite o 'n' Inicial: "))
            if(n1 < 2):
                print("\n[*] ATENÇÃO [*] O 'n' final não pode ser menor que 2!!!")
        
        while(serie <= 5 or serie >= 1 or serie >= n1):
            serie = int(input("\n====================================\n[+] Escolha uma das opções de série abaixo\n[1] - Série de Balmer\n[2] - Série de Lyman\n[3] - Série de Paschen\n[4] - Série de Brackett\n[5] - Série de Pfund\n====================================\n->"))
            if(serie > 5 or serie < 1):
                print("\n[*] ATENÇÃO [*] Digite um valor válido!!!")
            elif(serie >= n1):
                print("\n[*] ATENÇÃO [*] A série escolhida não pode ser maior ou igual ao n inicial!!!")
            else:
                break

        if(serie <= 5 or serie > 1):
            compFoton, freqFoton = calculaFoton(n1,serie)#Nas linhas 187 e 188 salvam em uma variavel auxiliar o retorno da função
            tipo_onda = calc_tipo_onda(freqFoton)
            print("\n[=] Um fóton do tipo %s foi emitido!" % tipo_onda)#Nas linhas 189 até a 191 elas printam no terminal os devidos resultados
            print("[=] Valor do comprimento de onda do fóton: %.2f nm" % compFoton)
            print("[=] Valor da frequência do fóton: ", f'{freqFoton:.2e}', end=" Hz\n" )
        elif(serie == 1):
            compFoton, freqFoton = calculaFoton(n1,serie)
            if(compFoton >= 410.2):
